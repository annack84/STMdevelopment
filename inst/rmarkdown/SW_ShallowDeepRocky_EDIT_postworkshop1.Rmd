...
author: "Anna C. Knight, Travis Nauman, Tara B. B. Bishop, and Michael C. Duniway"
date: "`r Sys.Date()`"
output: 
...
plot_data_first$PlantCommunity_fanny_output <- grp_fuzz
...
updated_cluster_numbers <- data.frame("PlantCommunity_fanny_output" = c(1, 2, 3, 4, 5, 6, 7, 8),
                                               "PlantCommunity_fuzzy_preworkshop" = c(1, 4, 5, 2, 6, 7, 3, 8))

plot_data_first <- left_join(plot_data_first, updated_cluster_numbers, by="PlantCommunity_fanny_output")

# merge Juniper-pinyon woodland with pinyon-juniper woodland for figures per 2023 manager workshop feedback
plot_data_first <- plot_data_first %>% 
  mutate(PlantCommunity_fuzzy = as.factor(ifelse(test = PlantCommunity_fuzzy_preworkshop %in% c(1, 5),
                                       yes = 9,
                                       no = PlantCommunity_fuzzy_preworkshop))) 
...
Clusters were reviewed by a group of US federal agency land managers from Utah and Colorado in Feburary 2023, and based on their feedback, two of the clusters (1 Juniper-pinyon woodland and 5 Pinyon-juniper woodland) were combined to become community phases of a single state (9 Pinyon-juniper woodland).
...
# 2D plotting
pal_veg2 <- RColorBrewer::brewer.pal(n=(k_groups_fuzzy-1), name = "Dark2")
...
group_labels_fuzzy <- paste0(group_labels_base,
                               ...
                               c(#nrow(filter(plot_data_first, PlantCommunity_fuzzy==1)),
                                 ...
                                 nrow(filter(plot_data_first, PlantCommunity_fuzzy==9))
                                  ),
                               ")")
...
# updated_cluster_names <- cbind(updated_cluster_numbers, 
#                                Community_names = c("Juniper-pinyon woodland",
...
#                                PlantCommunity_fuzzy = c(9, 4, 5, 2, 9, 7, 3, 8),
#                                State_names = c("Pinyon-juniper woodland",
#                                               "Open juniper-pinyon shrubland",
#                                               "C4 grassland",
#                                               "Annualized juniper savanna",
#                                               "Pinyon-juniper woodland",
...
updated_cluster_names <- cbind(updated_cluster_numbers, 
                               Community_names = c("Juniper-pinyon woodland",
                                                   ...
                                                   "C4 grassland",
                                              "Bare juniper savanna"),
                               PlantCommunity_fuzzy = c(9, 4, 9, 2, 6, 7, 3, 8),
                               State_names = c("Pinyon-juniper woodland",
                                                   "Annualized juniper savanna",
                                                   "Pinyon-juniper woodland",
                                                   "Open juniper-pinyon shrubland",
...
fuzzy_cluster_membership <- left_join(fuzzy_cluster_membership, updated_cluster_names, by = c("best_group" = "PlantCommunity_fanny_output"))

colnames(fuzzy_cluster_membership) <- c(paste0("Group_", updated_cluster_names$PlantCommunity_fuzzy_preworkshop, "_membership"),
                                        "PlotCode",
                                        "PlantCommunity_fanny_output",
                                        "Best_group",
                                        "Community_names",                 
...
#                                               "Sagebrush shrubland",
#                                               "Bare juniper savanna",
#                                               "Pinyon-juniper woodland")) # update with descriptive names after examining the groups
...
knitr::include_graphics("C:/Users/aknight/Documents/Telework_Backups/V_drive/ANNA_KNIGHT/ESG/STM/Analyses/Semiarid_Warm_ShallowDeepRocky_boxandarrow_workshop2.JPG")
```
...
plot_data_descriptive <- plot_data_pull(user=user,
  ...
  filter(!is.na(Month)) %>%
  left_join(select(plot_data_first, PlotCode, Year, PlantCommunity_fuzzy, PlantCommunity_fuzzy_preworkshop) , .)
...
# TODO why are 96 AIM plots missing gap data? is this a sampling date mismatch problem?? need to investigate
...
plot_data_first_tall <- select(plot_data_first,
                                             ...
                                             #PlantCommunity,
                                             PlantCommunity_fuzzy,
                                             PlantCommunity_fuzzy_preworkshop,
                                             AH_C3IntroducedPerenGrassCover, 
...
## State 2
## Open Juniper-Pinyon Shrubland State

## Community 2.1
## Open Juniper-Pinyon Shrubland State: Mixed shrubs with open Juniper-Pinyon

TODO write state description

```{r comm2-1-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp2 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==2),
...
## Community 3.1
## C4 Grassland State: C4 grassland with mixed shrubs and scattered Juniper-Pinyon

TODO write state description

```{r comm3-1-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp3 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==3),
                                   species_cols = colnames(descriptive.df),
                                   user = user)
# save the complete table for field crews
...
## State 4
## Annualized Juniper Savanna State

TODO write state description

## Community 4.1
## Annualized Juniper Savanna State: Juniper-pinyon savanna with invasive annual herbaceous understory

TODO write state description

```{r comm4-1-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp4 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==4),
                                   species_cols = colnames(descriptive.df),
                                   user = user)
# save the complete table for field crews
...
## State 6
## Gambel Oak Woodland State

TODO write state description

## Community 6.1
## Gambel Oak Woodland State: Gambel oak woodland with mixed shrubs and grasses

TODO write state description

```{r comm6-1-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp6 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==6),
                                   species_cols = colnames(descriptive.df),
                                   user = user)
# save the complete table for field crews
...
## State 7
## Sagebrush Shrubland State

TODO write state description

## Community 7.1
## Sagebrush Shrubland State: Sagebrush with mixed C3 grass understory

TODO write state description

```{r comm7-1-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp7 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==7),
                                   species_cols = colnames(descriptive.df),
                                   user = user)
# save the complete table for field crews
...
## State 8
## Bare Juniper Savanna State

TODO write state description

## Community 8.1
## Bare Juniper Savanna State: Juniper-pinyon savanna with light C3 understory

TODO write state description

```{r comm8-1-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp8 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==8),
                                   species_cols = colnames(descriptive.df),
                                   user = user)
# save the complete table for field crews
...
## State 9
## Pinyon-Juniper Woodland State

TODO write state description

## Community 9.1
## Pinyon-Juniper Woodland State: Open juniper-pinyon woodland

TODO write community description

```{r comm1-1-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp1 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==1),
                                   species_cols = colnames(descriptive.df),
                                   user = user)

# fix weird names
dom_spp_grp1[dom_spp_grp1$SpeciesCode=="PSSP6", "ScientificName"] <- "Pseudoroegneria spicata"
...
format_tables_EDIT_style(data = dom_spp_grp1_pub,
                         caption = "Community 9.1 plant community composition") %>%
  collapse_rows(columns = 1, valign = "top")
```
...
## Community 9.2
## Pinyon-Juniper Woodland State: Open pinyon-Juniper woodland

TODO write community description

```{r comm1-2-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
dom_spp_grp5 <- get_dominant_spp(data = filter(plot_data_descriptive, PlantCommunity_fuzzy_preworkshop==5),
                                   species_cols = colnames(descriptive.df),
                                   user = user)

# fix weird names
dom_spp_grp5[dom_spp_grp5$SpeciesCode=="PSSP6", "ScientificName"] <- "Pseudoroegneria spicata"
...
format_tables_EDIT_style(data = dom_spp_grp5_pub,
                         caption = "Community 9.2 plant community composition") %>%
  collapse_rows(columns = 1, valign = "top")
```
...
mtbs_plots <- st_join(x=fuzzy_cluster_memb_sf, y=mtbs) %>%
  select(PlotCode, PlantCommunity_fuzzy, State_names, DateSampled, Ig_Date, geometry) %>%
  mutate(Burned_presampling = ifelse(test = Ig_Date < DateSampled,
                              yes = TRUE,
                              no = FALSE)) %>%
  group_by(PlotCode) %>%
  summarise(PlantCommunity_fuzzy = first(PlantCommunity_fuzzy),
...
mtbs_plots$PlantCommunity_fuzzy_reordered <- factor(mtbs_plots$PlantCommunity_fuzzy,
...
FireFeatureData_plots <- st_join(x=fuzzy_cluster_memb_sf, y=FireFeatureData) %>%
  select(PlotCode, PlantCommunity_fuzzy, State_names, DateSampled, Fire_Year, geometry) %>%
...
#"AIM_Colorado NWDO White River FO 2018_WyoSage-697"

FireFeatureData_plots[FireFeatureData_plots$PlotCode=="AIM_Colorado NWDO White River FO 2018_WyoSage-697", "Years_burn_to_sampling"] <-
  year(FireFeatureData_plots[FireFeatureData_plots$PlotCode=="AIM_Colorado NWDO White River FO 2018_WyoSage-697", ]$DateSampled) - 
  FireFeatureData_plots[FireFeatureData_plots$PlotCode=="AIM_Colorado NWDO White River FO 2018_WyoSage-697", ]$Date_burned_1

FireFeatureData_plots[FireFeatureData_plots$PlotCode=="AIM_Colorado NWDO White River FO 2018_WyoSage-697", "Times_burned_presampling"] <- 0
FireFeatureData_plots[FireFeatureData_plots$PlotCode=="AIM_Colorado NWDO White River FO 2018_WyoSage-697", "Times_burned_postsampling"] <- 1
...
FireFeatureData_plots$PlantCommunity_fuzzy_reordered <- factor(FireFeatureData_plots$PlantCommunity_fuzzy,
                                                               levels = as.character(c(2:4, 6:9)),
                                                               labels = c(#"Juniper-pinyon woodland",
                                                                            "Open juniper-pinyon shrubland",
                                                                            "C4 grassland",
...
FireFeatureData_plots$PlantCommunity_fuzzy <- factor(FireFeatureData_plots$PlantCommunity_fuzzy,
                                                     levels = levels(plot_data_descriptive$PlantCommunity_fuzzy))
...
plots_per_group <- data.frame(PlantCommunity_fuzzy = c(2:4, 6:9),
...
pj_plot_pct <- pj_plots_cats %>%
  ...
  tidyr::pivot_longer(cols = c(MechanicalUnseeded, MechanicalSeeded, ChemicalUnseeded, ChemicalSeeded),
                      ...
                      values_to = "pct") %>%
  mutate(PlantCommunity_fuzzy = as.factor(PlantCommunity_fuzzy))

# add in 0s for states that had no treatments
if(any(!(sort(unique(plot_data_descriptive$PlantCommunity_fuzzy)) %in%
         sort(unique(pj_plot_pct$PlantCommunity_fuzzy))))){
    pj_missing_states <- sort(unique(plot_data_descriptive$PlantCommunity_fuzzy))[which(!(sort(unique(plot_data_descriptive$PlantCommunity_fuzzy)) %in% sort(unique(pj_plot_pct$PlantCommunity_fuzzy))))]
  
  treatmentpairs <- sort(unique(pj_plot_pct$TreatmentPair))
  
  pj_missing_df <- data.frame("PlantCommunity_fuzzy" = c(rep(pj_missing_states[1], length(treatmentpairs)),
                                                         rep(pj_missing_states[2], length(treatmentpairs))),
                              "TreatmentPair" = rep(treatmentpairs, length(pj_missing_states)),
                              "pct" = rep(0, length(treatmentpairs)*length(pj_missing_states))
                              )
  
  pj_plot_pct <- bind_rows(pj_plot_pct, pj_missing_df) %>% 
    mutate(PlantCommunity_fuzzy = factor(PlantCommunity_fuzzy,
                                         levels = sort(unique(plot_data_descriptive$PlantCommunity_fuzzy))))
}

treatments_plot_pcts <- ggplot(data = pj_plot_pct, aes(x = PlantCommunity_fuzzy, y = pct, fill = TreatmentPair)) +
  geom_col() +
...
GAP_proportions_byclass <- plot_data_descriptive %>%
  ...
  summarise(pct_JPwoodland = (length(which(PlantCommunity_fuzzy_reordered == "Juniper-pinyon woodland"))/n())*100,
            ...
            pct_SagebrushShrubland = (length(which(PlantCommunity_fuzzy_reordered == "Sagebrush shrubland"))/n())*100,
