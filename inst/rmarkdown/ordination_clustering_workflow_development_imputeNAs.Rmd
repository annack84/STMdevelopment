---
title: "Exploring Clustering and Ordination Techniques with Data Imputation"
author: "Anna Knight"
date: "1/26/2022"
output: 
...
#                 "CP_percent_100plus", # Canopy gaps > 100 cm TODO do we want annual or perennial gaps?
#                 "CP_percent_200plus",
...
ivlabs_list <- foreach(i=2:12, .packages = "labdsv") %dopar% indval(x=ord.df, clustering =grps[,i], numitr = 1000)
...
```{r silhouette-results, echo=FALSE, warning=FALSE, message=FALSE, fig.width=5, fig.height=5, fig.cap="Silhouette plot of the final groupings."}
...
spec_scores1 <- scores(ord, display = "species") %>%
  as.data.frame()
...
library(clValid)
res_clValid <- clValid(obj = norm.df, nClust = 2:12, clMethods = "fanny",
...
I modified the NbClust code to make it work for fanny.
```{r NbClust-fanny, echo=FALSE, eval=FALSE}
...
```{r NbClust-kmeans-compare, echo=FALSE, eval=FALSE}
...
# The k-means results for NbClust are similar to the fanny results if the membership exponent is close to 1.
```
...
Based on a suite of cluster validation indices, the best number of clusters is `r best_n_clust` with membership exponent of `r best_memb_exp`. In this case, however, when we use this combination of number of clusters and membership exponent, the FANNY algorithm reduces the number of clusters to 2 to get crisp results, so we'll use 2 group in the final fuzzy clustering results. 

```{r fuzzy-cluster, echo=FALSE}
#k_groups_fuzzy <- best_n_clust
k_groups_fuzzy <- 2 # REMOVE THESE TWO LINES IF FANNY DOESN'T LUMP THINGS DOWN WHEN YOU GIVE IT THE ACTUAL BEST NUMBER OF GROUPS
best_memb_exp <- 1.1
...
ord.df.fuzzydrop <- select(ord.df.fuzzydrop, -any_of(c("UNKS", "UNKPG", "UNKAF", "UNKPF", "PG1", "PG01", "AF1")))
...
# Do the clustering groups split by data source or SGU?

```{r clusters-by-datasource, echo=FALSE, fig.height=5, fig.width=5, fig.cap="Group membership by data source"}

plot_memb_by_source <- select(plot_data_first, SourceKey, PlotCode, Group1memb, Group2memb#, Group3memb
                              ) %>%
...
```{r clusters-by-sgu, echo=FALSE, warning=FALSE. message=FALSE, fig.height=5, fig.width=5, fig.cap="Group membership by SGU"}
...
plot_memb_by_sgu <- select(plot_data_first_sgu, SGU, PlotCode, Group1memb, Group2memb#, Group3memb
                              ) %>%
  tidyr::pivot_longer(cols = c(Group1memb, Group2memb#, Group3memb
                               ), names_to = "Group")

anovamod1 <- aov(Group1memb~SGU, data = plot_data_first_sgu)
summary(anovamod1)
tukey1 <- TukeyHSD(x=anovamod1)
labels1 <- multcompView::multcompLetters4(anovamod1, tukey1)$SGU

anovamod2 <- aov(Group2memb~SGU, data = plot_data_first_sgu)
summary(anovamod2)
tukey2 <- TukeyHSD(x=anovamod2)
labels2 <- multcompView::multcompLetters4(anovamod2, tukey2)$SGU

# yes, the group membership values are significantly different for the SGUs!

ggplot(data = plot_memb_by_sgu, aes(x=SGU, y=value, fill = Group)) +
  geom_boxplot() +
  #scale_fill_manual() +
  ylab("Group membership coefficient") +
  xlab("SGU")
...
#rmarkdown::render(input = "C:/Users/aknight/Desktop/Telework_Backups/V_drive/PROJECTS/ESG/STM/Scripts/STMdevelopment/R/ordination_clustering_workflow_development_imputeNAs.Rmd", output_format = "html_document", output_file = "C:/Users/aknight/Desktop/Telework_Backups/V_drive/PROJECTS/ESG/STM/Analyses/ordination_clustering_workflow_development_imputeNAs_20220131.html")
```
