...
subtitle: 'Supplemental material for "Quantifying regional ecological dynamics using agency monitoring data, ecological site descriptions and ecological site groups" by Duniway et al. (in review)'
author: 
  - Prepared by Anna Knight
  - Michael C. Duniway, US Geological Survey, Southwest Biological Science Center, Moab, UT
  - Anna Knight, US Geological Survey, Southwest Biological Science Center, Moab, UT
  - Travis Nauman, USDA National Soil Survey Center, Moab, UT
  - Tara B. B. Bishop, Utah Valley University, Department of Earth Science, Orem, UT
  - Sarah E. McCord, USDA-ARS Jornada Experimental Range, Las Cruces, NM
  - Nicholas P. Webb, USDA ARS, Southwest Watershed Research Center, Tucson, AZ
  - Joel T. Humphries, Bureau of Land Management, Colorado State Office, Lakewood, CO
...
```{r map, eval=TRUE, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE, fig.cap="Mapped extent of the Ecological Site Group (ESG). Areas shown in blue indicate the maximum mapped extent of this ESG (Nauman et al., 2022). Other ESGs likely occur within the highlighted areas. It is also possible for this ESG to occur outside of highlighted areas if detailed soil survey has not been completed or recently updated.", fig.width=7.48, fig.height=7.48}
...
```{r esd-data-pull, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
...
```{r soil-lattice, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE, fig.cap="Soil properties by depth as summarized from correlated map unit components in SSURGO (Nauman et al., 2022; Soil Survey Staff, 2022)"}
...
```{r ref-production, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE, fig.cap="Reference state production by plant functional group (based on reference states from Ecological Site Descriptions within this ESG, Nauman et al. 2022)"}
...
```{r ordination-plots, echo=FALSE, fig.height=8, fig.width=8, fig.cap="NMDS ordination of plot indicator data. Points represent plots; ellipses represent plant communities identified by the fuzzy clustering analysis."}
# 2D plotting
...
```{r save-fuzzy-cluster, echo=FALSE, eval=TRUE, error=FALSE, message=FALSE}
...
```{r custom-stm, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE, fig.cap="STM", fig.height=7, fig.width=7.48}
...
```{r custom-stm-legend, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE, fig.cap="STM Legend", fig.height=5, fig.width=5.51}
...
```{r plant-groups, echo=FALSE, message=FALSE, warning=FALSE, fig.height=16, fig.width=8, fig.cap="Plant and ground cover by state. Plant cover values (a-d) were measured using line-point intercept (LPI; Herrick et al., 2017); Ground cover values (e) were measured using LPI (for bare soil, litter, and lichens and mosses) and canopy gap methods (annual and perennial canopy gaps >100 cm and annual and perennial canopy gaps >200 cm; Herrick et al., 2017)"}
...
```{r groups-climate, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5.51, fig.width=5.51, fig.cap="Climate gradients by state. Larger aridity index values indicate a less arid condition. Aridity index data from Trabucco and Zomer (2018); precipitation and temperature data from PRISM Climate Group (2020)"}
# group_labels_fuzzy <- paste0(group_labels_base,
#                                " (n = ",
...
```{r groups-soils, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=7.48, fig.cap="Soil texture, rock fragment content, and depth class by state, based on digital soil maps (Nauman and Duniway, 2020; Brungard et al., 2021)."}
# soil depth figure
...
```{r groups-diversity, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=7.48, fig.cap='(a) Species diversity (Shannon diversity index, calculated using the "vegan" package in R; Oksanen et al., 2022), (b) species richness, and (c) percent native plant cover out of total foliar cover by state. Values are based on line-point intercept data, so they may underestimate diversity compared to a full species inventory. Higher diversity values indicate greater diversity (0 indicates a plot that has only one species).'}
# group_labels_fuzzy <- paste0(group_labels_base,
#                                " (n = ",
...
```{r groups-erosion, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=5.51, fig.cap="Erosion potential by state. Modelled wind erosion (a) is based on the Aeolian EROsion (AERO) model and is only available for the subset of plots where annual and perennial canopy gap data were collected (Edwards et al., 2022). Soil stability data (b) were collected following Herrick et al. (2017) at field plot locations (Nusser and Goebel, 1997; Toevs et al., 2011; Witwicki et al., 2017). Runoff (c, e) and soil loss (d, f) were modeled using the Rangeland Hydrology and Erosion Model (RHEM, Hernandez et al. 2017). Soil erosion model results were accessed from the Landscape Data Commons (McCord et al., 2023)"}
...
Brungard, C., Nauman, T., Duniway, M., Veblen, K., Nehring, K., White, D., Salley, S., and Anchang, J. (2021). Regional ensemble modeling reduces uncertainty for digital soil mapping. _Geoderma_ 397:114998.

Calinski T., and Harabasz J. (1974). A Dendrite Method for Cluster Analysis. _Communications in Statistics – Theory and Methods_, 3(1), 1–27.

Davies D.L., and Bouldin D.W. (1979). A cluster separation measure. _IEEE Transactions on Pattern Analysis and Machine Intelligence_, 1(2), 224–227.

Duniway, M. C., A. Knight, T. Nauman, T. B. B. Bishop, S. E. McCord, N. P. Webb, C. J. Williams, and J. T. Humphries. In review. Quantifying regional ecological dynamics using agency monitoring data, ecological site descriptions, and ecological site groups. _Rangeland Ecology and Management_.

Dunn, J. (1974). Well separated clusters and optimal fuzzy partitions. _Journal Cybernetics_, 4(1), 95–104.

Edwards, B. L., N. P. Webb, M. S. Galloza, J. W. Van Zee, E. M. Courtright, B. F. Cooper, L. J. Metz, J. E. Herrick, G. S. Okin, M. C. Duniway, J. Tatarko, N. H. Tedala, D. N. Moriasi, B. A. Newingham, F. B. Pierson, D. Toledo, and R. S. Van Pelt. 2022. Parameterizing an aeolian erosion model for rangelands. _Aeolian Research_ 54:100769.

Hernandez, M., M. A. Nearing, O. Z. Al-Hamdan, F. B. Pierson, G. Armendariz, M. A. Weltz, K. E. Spaeth, C. J. Williams, S. K. Nouwakpo, D. C. Goodrich, C. L. Unkrich, M. H. Nichols, and C. D. Holifield Collins. 2017. The Rangeland Hydrology and Erosion Model: A Dynamic Approach for Predicting Soil Loss on Rangelands. _Water Resources Research_ 53:9368-9391.

Herrick, J. E., Van Zee, J. W., McCord, S. E., Courtright, E. M., Karl, J. W., & Burkett, L. M. (2017). _Monitoring Manual for Grassland, Shrubland, and Savanna Ecosystems (Second ed. Vol. 1: Core Methods)._ Las Cruces, New Mexico: USDA-ARS Jornada Experimental Range.

Hubert L.J., Levin J.R. (1976). A General Statistical Framework for Assessing Categorical Clustering in Free Recall. _Psychological Bulletin_, 83(6), 1072–1080.

Kaufman, L., and P. J. Rousseeuw. 1990. Finding groups in data: an introduction to cluster analysis. _Probability and Mathematical Statistics. Applied Probability and Statistics._

Maechler, M., Rousseeuw, P., Struyf, A., Hubert, M., Hornik, K.(2021).  _cluster: Cluster Analysis Basics and Extensions._ R package version 2.1.2.

McCord, S. E., Brehm, J. R., Burnett, S. H., Dietrich, C., Edwards, B., Metz, L. J., Hernandez Narvaez, M., Pierson, F., Ramirez, K. S., Stauffer, N. G., Webb, N. P., and Tweedie, C. E. (2022). A framework and toolset for standardizing agroecosystem indicators. _Ecological Indicators_, 144. doi:10.1016/j.ecolind.2022.109511

McCord, S. E., Webb, N. P., Bestelmeyer, B. T., Bonefont, K., Brehm, J. R., Brown, J., Courtright, E. M., Dietrich, C., Duniway, M. C., Edwards, B., Fraser, C. Herrick, J. E., Knight, A. C., Metz, L., Van Zee J. W., and Tweedie, C. (2023). "The Landscape Data Commons: A system for standardizing, accessing, and applying large environmental datasets for agroecosystem research and management." _Agricultural & Environmental Letters 8_(2).

Milligan G.W. (1980). An Examination of the effect of six types of error perturbation on fifteen clustering algorithms. _Psychometrika_, 45(3), 325–342.

Milligan G.W. (1981). A Monte Carlo study of thirty internal criterion measures for cluster analysis. _Psychometrika_, 46(2), 187–199.

Nauman, T. W., and M. C. Duniway. 2020. A hybrid approach for predictive soil property mapping using conventional soil survey data. _Soil Science Society of America Journal_ 84:1170-1194.
...
Nusser, S. M., and J. J. Goebel. (1997). The National Resources Inventory: A long-term multi-resource monitoring programme. _Environmental and Ecological Statistics_ 4:181-204.
...
Rousseeuw, P. (1987). Silhouettes: A graphical aid to the interpretation and validation of cluster analysis. _Journal of Computational and Applied Mathematics_, 20, 53–65.
...
Toevs, G. R., Karl, J. W., Taylor, J. J., Spurrier, C. S., Karl,  M. S., Bobo, M. R., and Herrick, J. E. (2011). Consistent indicators and methods and a scalable sample design to meet assessment, inventory, and monitoring information needs across scales. _Rangelands_ 33:14-20.
...
```{r save-data-versions, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
# # save data versions
