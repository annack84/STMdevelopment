...
```{r echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
esds <- esd_data_pull("Travis")
comps <- comp_data_pull("Travis")
num_esds <- nrow(esds)
```
This ecological site group includes `r num_esds` ecological sites. These sites were correlated to the group based on average soil, climate and geomorphic properties of the soil components linked to each site.
```{r echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
...
# Note from Travis: Could also summarize the ESG referece production indicators with boxplot
resp_vars <- colnames(esds)[1:21]
resp_vars <- resp_vars[!grepl("ecoclassid", resp_vars)]
...
phys.df <- dplyr::tribble(~Item, ~Description,
                          "Landforms", as.character(lfsum)[1:10],
                          "Slope", slope_field)
