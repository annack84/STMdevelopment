---
title: "Draft Clustering and Ordination for Loamy Uplands only in the Semiarid Warm Sandy and Loamy Uplands ESG (by plant func. group)"
author: "Anna Knight"
date: "2/9/2022"
output: 
...
group_labels_LoamyUplands <- c("1 - C3 perennial grassland with sagebrush",
                    "2 - Bare soil/juniper woodland",
                    "3 - C4 perennial grassland",
...
```{r ordination-spplots, eval=FALSE, echo=FALSE, fig.height=10, fig.width=10, fig.cap="NMDS ordination of plot indicator data with plant species overlayed."}
...
points(ord, choices = c(1,2), display = "sites", # plot points - can choose "sites" or "species"
       col=pal_veg[plot_data_first_LoamyUplands$PlantCommunity],
       pch = 21, cex = .6, bg=pal_veg[plot_data_first_LoamyUplands$PlantCommunity])
ordiellipse(ord, plot_data_first_LoamyUplands$PlantCommunity, col=pal_veg, lwd = 2, label = T) # plot your groups
plot(ord.fit, add=T)
...
plot(ord, type = "n", axes=FALSE,
     ...
     ylab = "")
legend(x="center",
       legend = group_labels_LoamyUpland,
       fill = pal_veg,
       title = "Plant communities")
...
leaflet.minicharts::addMinicharts(map = EDIT_map(target_ESG = target_ESG, user = user, maxZoomStyle = "developer"),
                                  lng = plot_data_first_LoamyUplands2$Longitude_NAD83,
                                    ...
                                    chartdata = plot_data_first_LoamyUplands2[ , c("Group1memb", "Group2memb"#, "Group3memb"
                                                                     )],
                                    type = "pie",
...
plot_memb_by_source <- select(plot_data_first_LoamyUplands2, SourceKey, PlotCode, Group1memb, Group2memb#, Group3memb
                              ) %>%
...
plot_memb_by_sgu <- select(plot_data_first_LoamyUplands3, UCRB_SGUs_ProbMax, PlotCode, Group1memb, Group2memb#, Group3memb
                              ) %>%
  tidyr::pivot_longer(cols = c(Group1memb, Group2memb#, Group3memb
                               ), names_to = "Group")
