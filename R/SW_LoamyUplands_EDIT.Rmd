...
# SET EVAL TO TRUE WHEN THIS TABLE GETS UPDATED!
...
```{r nbclust-fanny-loop, eval=FALSE, warning=FALSE}
...
```{r ordination-scree, eval=FALSE, error=FALSE, warning=FALSE, message=FALSE, results='hide', fig.keep='all', cache=FALSE, fig.cap="NMDS ordination scree plot comparing number of dimensions in the solution to stress."}
...
group_labels_fuzzy <- c("1 - Depauperate C4
perennial grassland",
                        "2 - Bare ground",
                        "3 - Sagebrush/C3 perennial
grass savanna",
...
ordiellipse(ord, plot_data_first$PlantCommunity_fuzzy, col=pal_veg2, lwd = 2, label = T,
...
```{r plant-groups, echo=FALSE, message=FALSE, warning=FALSE, fig.height=14, fig.width=10, fig.cap="Plant cover by state"}
...
row.names(mean_cover)[which(mean_cover$V1>=0.5)]
...
woody_grps <- c("AH_ArtemisiaTridentataCover", "CHVI8", "SYRO", "ERNA10", "SAVE4",
                "PUTR2", "GUSA2", "PIED", "JUOS", "QUGA", "AMUT", "AMAL2")
woody_cover_states <- ggplot(data = filter(plot_data_first_tall, CoverType %in% woody_grps),
  ...
  theme_bw()
woody_legend <- cowplot::get_legend(woody_cover_states)
...
perenherb_cover_states <- ggplot(data = filter(plot_data_first_tall, CoverType %in% perenherb_grps),
  ...
  theme_bw()
perenherb_legend <- cowplot::get_legend(perenherb_cover_states)
...
annherb_cover_states <- ggplot(data = filter(plot_data_first_tall, CoverType %in% annherb_grps),
  ...
  theme_bw()
annherb_legend <- cowplot::get_legend(annherb_cover_states)
...
ground_legend <- cowplot::get_legend(ground_cover_states)
...
cover_states <- cowplot::plot_grid(woody_cover_states + theme(legend.position = "none"),
                   ...
                   ncol = 1, axis = "lr", rel_heights = c(1,1,1,1.45))

cover_axes <- cowplot::plot_grid(woody_legend,
                   perenherb_legend,
                   annherb_legend,
                   ground_legend,
                   ncol = 1, rel_heights = c(1,1,1,1.45))
...
```{r groups-totalfoliar, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=6.25, fig.cap="Total foliar cover by state"}
ggplot(data = plot_data_descriptive,
...
grp3_desc_data <- filter(plot_data_descriptive, PlantCommunity_fuzzy==3)

# consider including species present in >10% of plots:
names(colSums(sp_pa_desc)[which(colSums(sp_pa_desc)>(nrow(plot_data_descriptive)*0.10))])
# also consider including species with high mean cover across all plots:
mean_cover <- summarise(plot_data_descriptive, across(.cols = BRTE:QUERC, .fns = mean)) %>%
  select(-any_of(c("Longitude_NAD83", "Latitude_NAD83", "Year", "Month", "Day"))) %>%
  t() %>%
  as.data.frame()
