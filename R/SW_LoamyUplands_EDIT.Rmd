...
author: "Anna Knight and Travis Nauman"
date: "2/28/2022"
output: 
...
group_labels_fuzzy <- c(paste0("Depauperate C4 perennial
grassland (n = ", nrow(filter(plot_data_first, PlantCommunity_fuzzy==1)), ")"),
                        paste0("Bare ground (n = ", nrow(filter(plot_data_first, PlantCommunity_fuzzy==2)), ")"),
                        paste0("Sagebrush/C3 perennial
grass savanna (n = ", nrow(filter(plot_data_first, PlantCommunity_fuzzy==3)), ")"),
                        paste0("Gambel oak
...
```{r plant-groups, echo=FALSE, message=FALSE, warning=FALSE, fig.height=14, fig.width=12, fig.cap="Plant cover by community"}
...
cover_states <- cowplot::plot_grid(woody_cover_states + theme(legend.position = "none"),
...
cover_axes <- cowplot::plot_grid(cowplot::get_legend(woody_legend),
                   cowplot::get_legend(perenherb_legend),
                   cowplot::get_legend(annherb_legend),
                   cowplot::get_legend(ground_cover_states),
                   ncol = 1)

cowplot::plot_grid(cover_states, cover_axes, ncol = 2, rel_widths = c(1, 0.6))
```
...
```{r groups-totalfoliar, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=6.25, fig.cap="Total foliar cover by plant community"}
ggplot(data = plot_data_descriptive,
       aes(x=PlantCommunity_fuzzy, y=TotalFoliarCover)) +
  geom_boxplot() +
  scale_x_discrete(labels=group_labels_fuzzy) +
  xlab("Plant community") +
...
```{r groups-diversity, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=6.25, fig.cap="Species diversity (Shannon diversity index) by plant community. Values are based on line-point intercept data, so they may underestimate diversity compared to a full species inventory. Higher values indicate greater diversity (0 indicates a plot that has only one species)."}
ggplot(data = plot_data_descriptive,
...
```{r groups-richness, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=6.25, fig.cap="Species richness by plant community. Values are based on line-point intercept data, so they may underestimate diversity compared to a full species inventory."}
ggplot(data = plot_data_descriptive,
...
## Community 1.1
## Reference State: sagebrush/C3 perennial grass savanna
...
## Community 2.1
## Reference State: gambel oak shrubland
...
## Community 3.1
## Alternative State: Depauperate C4 perennial grassland
...
## State 4
## Annualized/bare state

TODO write state description

## Community 4.1
## Annualized/bare state: Bare ground
...
## Community 4.2
## Annualized/bare state: Non-native annual grassland

TODO write state description

```{r comm4-2-dominant-spp, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
...
format_tables_EDIT_style(data = dom_spp_grp5_pub,
                         caption = "Community 4.2 plant community composition") %>%
  collapse_rows(columns = 1, valign = "top")
```
